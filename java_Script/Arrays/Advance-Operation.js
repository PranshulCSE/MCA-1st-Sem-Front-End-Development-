...
const price=products.filter((products)=>products.price>50);
console.log(price);
// This will return only those Products whose Price are greater then 50.
// To Sort it in Descending Order we can write the following Command.
// const price=products.filter((products)=>products.price>50).sort((a,b)=>b.price-a.price);
// Above line will give output in Descending Order..

const List=products.map((products)=>({Name:products.name, Price:products.price}));
console.log(List);
// The Following Statement will return Array of Objects which Display the Name & Price of Product
