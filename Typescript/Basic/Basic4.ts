...
let mixedArray: (number | string | boolean)[] = [1, "Two", true, 4, "Five"];
...
let person={ name: "Pranshul", age: 22, isStudet: true };
...
let product: { productId: number; productName: string; price: number }={
...
let company: {
    companyName: string;
    location: string;
    employeeDetails: { id: number; name: string; isPermanent: boolean }[];
} = {
